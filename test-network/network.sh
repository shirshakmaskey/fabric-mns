...
function networkUp() {
  ...
  COMPOSE_FILES="-f ${COMPOSE_FILE_BASE} -f ${COMPOSE_CLI_BASE} -f ${ADDTIONAL_FILE_SERVER} -f ${GRAPHITE_SERVER}"
...
function networkDown() {
  ...
  docker-compose -f $COMPOSE_FILE_BASE -f $ADDTIONAL_FILE_SERVER -f ${COMPOSE_CLI_BASE} -f ${GRAPHITE_SERVER} -f $COMPOSE_FILE_COUCH -f $COMPOSE_FILE_CA down --volumes --remove-orphans
...
COMPOSE_CLI_BASE=docker/docker-compose-cli.yaml
GRAPHITE_SERVER=docker/docker-compose-graphite.yaml
ADDTIONAL_FILE_SERVER=../reputation-server/docker/docker-compose-python.yaml
